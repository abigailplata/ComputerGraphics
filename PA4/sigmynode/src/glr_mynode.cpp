...
void GlrMyNode::render ( SnShape* s, GlContext* ctx )
{
	...
	{	GsPnt o = c.init;
		...
		float phi = 0;
		float delta = ((2 * 3.14159265359f) / (res));
		

		if ( w<=0 || h<=0 ) return; // invalid parameters

		GsArray<GsVec> N(0,6); // will hold the points forming my triangles (size 0, but pre-allocate 6 spaces)
		GsArray<GsPnt> V(0,6); //!< List of vertex coordinates
		//GsArray<Face> F(0,6); //!< Triangular faces indices to V
		/*P.push() = o;
		...
		for (int i = 0; i < res; i++) {
			...
			for (int j = 0; j < res; j++) {
				...
				N.push() = GsVec(((R + r * cos(theta)) * cos(phi)), ((R + r * sin(theta)) * sin(phi)), (r * sin(theta)));
				N.push() = GsVec(((R + r * cos(theta)) * cos(phi + delta)), ((R + r * sin(theta)) * sin(phi + delta)), (r * sin(theta)));
				N.push() = GsVec(((R + r * cos(theta + delta)) * cos(phi)), ((R + r * sin(theta + delta)) * sin(phi)), (r * sin(theta + delta)));

				V.push() = GsPnt(((R + r * cos(theta)) * cos(phi)), ((R + r * sin(theta)) * sin(phi)), (r * sin(theta)));
				V.push() = GsPnt(((R + r * cos(theta)) * cos(phi + delta)), ((R + r * sin(theta)) * sin(phi + delta)), (r * sin(theta)));
				V.push() = GsPnt(((R + r * cos(theta + delta)) * cos(phi)), ((R + r * sin(theta + delta)) * sin(phi)), (r * sin(theta + delta)));

				//second triangle:
				N.push() = GsVec(((R + r * cos(theta + delta)) * cos(phi)), ((R + r * sin(theta + delta)) * sin(phi)), (r * sin(theta + delta)));
				N.push() = GsVec(((R + r * cos(theta)) * cos(phi + delta)), ((R + r * sin(theta)) * sin(phi + delta)), (r * sin(theta)));
				N.push() = GsVec(((R + r * cos(theta + delta)) * cos(phi + delta)), ((R + r * sin(theta + delta)) * sin(phi + delta)), (r * sin(theta + delta)));

				V.push() = GsPnt(((R + r * cos(theta + delta)) * cos(phi)), ((R + r * sin(theta + delta)) * sin(phi)), (r * sin(theta + delta)));
				V.push() = GsPnt(((R + r * cos(theta)) * cos(phi + delta)), ((R + r * sin(theta)) * sin(phi + delta)), (r * sin(theta)));
				V.push() = GsPnt(((R + r * cos(theta + delta)) * cos(phi + delta)), ((R + r * sin(theta + delta)) * sin(phi + delta)), (r * sin(theta + delta)));
			}
		}
		...
		glBufferData ( GL_ARRAY_BUFFER, N.sizeofarray(), N.pt(), GL_STATIC_DRAW );
		glVertexAttribPointer ( 0, 3, GL_FLOAT, GL_FALSE, 0, 0 );
		_psize = N.size(); // after this line array P will be deallocated (data has been sent to graphics card)
