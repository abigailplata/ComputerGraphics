...
void MyViewer::build_scene ()
{
	...
	SnModel* sn_hand;
	hand->load("C:/Users/aplata3/Downloads/pa3/arm/rhand.m");
	sn_hand = new SnModel(hand);
	SnGroup* hand_sng = new SnGroup;
	SnManipulator* hand_mani = new SnManipulator;
	hand_sng->add(sn_hand);
	hand_mani->child(hand_sng);
	rootg()->add(hand_mani);

	
	GsModel* upperarm = new GsModel();
	SnModel* sn_upperarm;
	upperarm->load("C:/Users/aplata3/Downloads/pa3/arm/rupperarm.m");
	sn_upperarm = new SnModel(upperarm);
	SnGroup* upperarm_sng = new SnGroup;
	SnManipulator* upperarm_mani = new SnManipulator;
	upperarm_sng->add(sn_upperarm);
	upperarm_mani->child(upperarm_sng);
	rootg()->add(upperarm_mani);

	GsModel* lowerarm = new GsModel();
	SnModel* sn_lowerarm;
	lowerarm->load("C:/Users/aplata3/Downloads/pa3/arm/rlowerarm.m");
	sn_lowerarm = new SnModel(lowerarm);
	SnGroup* lowerarm_sng = new SnGroup;
	SnManipulator* lowerarm_mani = new SnManipulator;
	lowerarm_sng->add(sn_lowerarm);
	lowerarm_mani->child(lowerarm_sng);
	rootg()->add(lowerarm_mani);
}
