	...
	{	GsPnt o = c.init;
		...
		for (int i = 0; i < res; i++) {
			...
			for (int j = 0; j < res; j++) {
				...
				P.push() = GsVec(((R + r * cos(theta)) * cos(phi)), ((R + r * sin(theta)) * sin(phi)), (r * sin(theta)));
				P.push() = GsVec(((R + r * cos(theta)) * cos(phi + delta)), ((R + r * sin(theta)) * sin(phi + delta)), (r * sin(theta)));
				P.push() = GsVec(((R + r * cos(theta + delta)) * cos(phi)), ((R + r * sin(theta + delta)) * sin(phi)), (r * sin(theta + delta)));

				//second triangle:
				P.push() = GsVec(((R + r * cos(theta + delta)) * cos(phi)), ((R + r * sin(theta + delta)) * sin(phi)), (r * sin(theta + delta)));
				P.push() = GsVec(((R + r * cos(theta)) * cos(phi + delta)), ((R + r * sin(theta)) * sin(phi + delta)), (r * sin(theta)));
				P.push() = GsVec(((R + r * cos(theta + delta)) * cos(phi + delta)), ((R + r * sin(theta + delta)) * sin(phi + delta)), (r * sin(theta + delta)));
			}
		}
